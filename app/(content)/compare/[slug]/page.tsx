    ...
    <div className="min-h-screen bg-white">
      ...
      <main>
        ...
        <section className="py-12">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-2xl font-bold text-[#1A1A2E] mb-6">Quick Comparison</h2>
              ...
              <table className="w-full">
                <thead>
                  <tr>
                    ...
                    {comp.competitors.map((c, i) => (
                      <th
                        key={c.name}
                        className={`p-4 text-center font-bold ${i === 0 ? 'bg-[#1A1A2E] text-white' : 'bg-gray-50 text-gray-600'}`}
                      >
                ...
                <tbody>
                  <tr className="border-t border-gray-100">
                    ...
                    {comp.competitors.map((c, i) => (
                      <td key={c.name} className={`p-4 text-center text-sm ${i === 0 ? 'bg-green-50/50 border-x border-[#00C853]/20 font-bold text-[#1A1A2E]' : 'text-gray-600'}`}>
                        {c.monthlyPrice}
                      </td>
                    ))}
                  </tr>
                  ...
                  <tr className="border-t border-gray-100">
                    ...
                    {comp.competitors.map((c, i) => (
                      <td key={c.name} className={`p-4 text-center ${i === 0 ? 'bg-green-50/50 border-x border-[#00C853]/20' : ''}`}>
                        {c.contractRequired ? <span className="text-gray-500 text-sm">Yes</span> : <span className="text-[#00C853] font-semibold text-sm">Flexible</span>}
                      </td>
                    ))}
                  </tr>
                  <tr className="border-t border-gray-100 bg-gray-50/50">
                  ...
                  {comp.competitors[0].features.map((feature, fi) => (
                    <tr key={feature} className={`border-t border-gray-100 ${fi % 2 === 0 ? '' : 'bg-gray-50/50'}`}>
                      ...
                      {comp.competitors.map((c, i) => (
                        <td key={c.name} className={`p-4 text-center ${i === 0 ? 'bg-green-50/50 border-x border-[#00C853]/20' : ''}`}>
                          <Check size={18} className={i === 0 ? 'text-[#00C853] mx-auto' : 'text-gray-400 mx-auto'} />
                        </td>
                      ))}
                    </tr>
                  ))}
        ...
        <section className="py-12 bg-[#F8F9FA]">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-2xl font-bold text-[#1A1A2E] mb-8">Pros & Cons</h2>
            <div className={`grid gap-6 ${comp.competitors.length <= 2 ? 'md:grid-cols-2' : 'md:grid-cols-2 lg:grid-cols-3'}`}>
              {comp.competitors.map((c, i) => (
                <div key={c.name} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                  <div className={`p-4 ${i === 0 ? 'bg-[#1A1A2E]' : 'bg-gray-100'}`}>
                    <div className="flex items-center gap-2">
                      {i === 0 && <Crown size={16} className="text-yellow-400" />}
                      <h3 className={`font-bold text-lg ${i === 0 ? 'text-white' : 'text-[#1A1A2E]'}`}>{c.name}</h3>
                    </div>
                    <div className="mt-1"><StarRating rating={c.rating} /></div>
                  </div>
                  <div className="p-5 space-y-4">
                    <div>
                      <h4 className="font-semibold text-[#00C853] text-sm uppercase tracking-wider mb-2">Pros</h4>
        ...
        <section className="py-12 bg-[#F8F9FA]">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="bg-white rounded-2xl border-2 border-[#00C853] p-8 text-center">
              ...
              <h2 className="text-2xl font-bold text-[#1A1A2E] mb-3">Our Verdict</h2>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                ...
                <a href={`tel:${PHONE_NUMBER_RAW}`} className="flex items-center gap-2 text-gray-500 hover:text-[#1A1A2E] transition-colors text-sm">
                  <Phone size={16} />
