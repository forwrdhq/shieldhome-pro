...
// Step A schema: phone-first (required)
...
// Step B schema: email + zip (optional enrichment)
const stepBSchema = z.object({
  email: z.string().email('Valid email required'),
  zipCode: z.string().regex(/^\d{5}$/, 'Valid 5-digit ZIP required'),
})

type StepAFormData = z.infer<typeof stepASchema>
type StepBFormData = z.infer<typeof stepBSchema>

export interface StepASubmitData extends StepAFormData {
  tcpaConsent: boolean
}

export interface StepBSubmitData extends StepBFormData {}

interface QuizContactCaptureProps {
  totalQuestions: number
  quizAnswers: QuizAnswers
  onSubmitStepA: (data: StepASubmitData) => Promise<void>
  onSubmitStepB: (data: StepBSubmitData) => Promise<void>
  onSkipStepB: () => void
  loadingA: boolean
  loadingB: boolean
  stepAComplete: boolean
}
...
export default function QuizContactCapture({
  ...
  const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setValueA('phone', digits, { shouldValidate: true })
  }

  const onStepASubmit = (data: StepAFormData) => {
    onSubmitStepA({ ...data, tcpaConsent: true })
  }

  const onStepBSubmit = (data: StepBFormData) => {
    onSubmitStepB(data)
  }
    ...
    <div className="min-h-screen bg-white flex flex-col px-5 md:px-8 pt-6 pb-8">
      ...
      {!stepAComplete ? (
        ...
        >
          ...
          {/* Step A Form */}
          <form onSubmit={handleSubmitA(onStepASubmit)} className="w-full space-y-3">
            <div>
              <input
                {...registerA('firstName')}
                type="text"
              ...
              />
              {errorsA.firstName && (
                <p className="text-[11px] text-red-500 mt-1 ml-1 font-body">{errorsA.firstName.message}</p>
              )}
            </div>
            ...
            <div>
              <input
                ...
                className="w-full px-4 py-3 rounded-lg border border-slate-200 text-slate-900 placeholder-slate-400 text-[16px] font-body focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent bg-white"
              />
              <input type="hidden" {...registerA('phone')} />
              {errorsA.phone && (
                <p className="text-[11px] text-red-500 mt-1 ml-1 font-body">{errorsA.phone.message}</p>
              )}
            </div>
          ...
          <div className="flex flex-col items-center gap-2 mt-6">
            <div className="flex items-center gap-2 text-[11px] text-slate-400 font-body">
              ...
              <span>Trusted by 2M+ American families | Vivint Authorized Dealer</span>
            </div>
            <div className="flex items-center gap-2 text-[11px] text-slate-400 font-body">
        ...
        /* ═══ STEP B: Email + ZIP (Optional Enrichment) ═══ */
        <div
          className="flex-1 flex flex-col items-center justify-center max-w-md mx-auto w-full"
          style={{ animation: 'fadeUp 500ms cubic-bezier(0.16, 1, 0.3, 1) both' }}
        >
          ...
          <p className="text-[13px] text-slate-500 font-body text-center mb-6">
            One more thing &mdash; where should we email your detailed security report?
          </p>

          {/* Step B Form */}
          <form onSubmit={handleSubmitB(onStepBSubmit)} className="w-full space-y-3">
            <div>
              <input
                {...registerB('email', {
              ...
              />
              {errorsB.email && (
                <p className="text-[11px] text-red-500 mt-1 ml-1 font-body">{errorsB.email.message}</p>
              )}
            </div>
            ...
            <div>
              <input
                {...registerB('zipCode', {
                  ...
                  pattern: { value: /^\d{5}$/, message: 'Valid 5-digit ZIP required' },
                })}
                type="text"
              ...
              />
              {errorsB.zipCode && (
                <p className="text-[11px] text-red-500 mt-1 ml-1 font-body">{errorsB.zipCode.message}</p>
              )}
            </div>
          ...
          <button
            onClick={onSkipStepB}
            className="mt-4 text-[13px] text-slate-400 hover:text-emerald-600 font-body transition-colors cursor-pointer"
          >
            Skip &mdash; just show my score &rarr;
          </button>
        </div>
      )}
    </div>
