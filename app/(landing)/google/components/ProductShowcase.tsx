...
const products = [
  {
    ...
    videoId: 'indoor-camera-pro', // YouTube search fallback
...
export default function ProductShowcase() {
  ...
  const handleCardClick = useCallback((product: typeof products[0]) => {
    if (product.youtubeId) {
      ...
      window.dataLayer?.push({ event: 'product_video_play', product: product.title })
    } else {
      // No video — scroll to form
