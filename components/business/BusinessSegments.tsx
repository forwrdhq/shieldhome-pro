...
import { ShoppingBag, Building2, Factory, UtensilsCrossed, AlertTriangle } from 'lucide-react'
      ...
      <div className="max-w-5xl mx-auto px-5 md:px-12">
        <div ref={headingRef} className="text-center mb-8 md:mb-10">
          ...
          <p className="text-[13px] md:text-[15px] font-body text-slate-400">
            Select your business type. We built a different solution for each one.
          </p>
        </div>
        ...
        <div className="flex overflow-x-auto gap-2 mb-6 pb-1 scrollbar-hide">
          {segments.map((seg, i) => (
            <button
              ...
              className={`flex-shrink-0 flex items-center gap-2 px-4 py-2.5 rounded-full border text-[12px] md:text-[13px] font-heading font-semibold transition-all duration-200 ${
                activeTab === i
                  ? 'border-emerald-500 bg-emerald-500 text-white shadow-[0_4px_12px_rgba(5,150,105,0.3)]'
                  : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300 hover:bg-slate-50'
              }`}
            >
              <span className={activeTab === i ? 'text-white' : 'text-slate-400'}>{seg.icon}</span>
        ...
        <div className="border border-slate-200 rounded-2xl overflow-hidden">
          ...
          <div className="bg-slate-900 px-6 py-4 flex items-center gap-3">
            ...
            <p className="text-[13px] md:text-[14px] font-heading font-semibold text-white">
              {active.stat}
            </p>
          </div>
          ...
          <div className="p-5 md:p-7">
            ...
            <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 md:p-5 mb-5">
              <p className="text-[10px] md:text-[11px] font-heading font-semibold uppercase tracking-[0.12em] text-emerald-700 mb-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 text-emerald-600 hover:text-emerald-700 font-heading font-semibold text-[13px] md:text-[14px] transition-colors duration-200"
            >
              {active.cta} →
            </button>
          </div>
